...
export default function PrasadManagementScreen() {
  ...
  useEffect(() => {
    const fetchFestivals = async () => {
      try {
        const response = await fetch(`http://${HOST}:${PORT}/api/prasadam/festivals`);
...
const handleDelete = async (festivalId) => {
  try {
    ...
    const response = await fetch(`http://${HOST}:${PORT}/api/prasadam/festivals/soft-delete/${festivalId}`, {
      method: 'PATCH',
      headers: {
