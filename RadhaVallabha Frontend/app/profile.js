...
const handlePickImage = async () => {
  ...
  try {
    ...
    const res = await fetch(`http://192.168.1.43:3000/api/upload-profile-image/${userId}`, {
      method: 'POST',
      body: formData,
      headers: {
...
const updateUserProfile = async () => {
  ...
  try {
    ...
    const res = await fetch(`http://192.168.1.43:3000/api/update-profile/` + userId, {
        method: "PUT",
        headers: {
        ...
        if (res.ok) {
            ...
            const updatedUser = { ...user, phone };//set safeuser in future
            setUser(updatedUser);
            await AsyncStorage.setItem("userData", JSON.stringify(updatedUser));
        } else {
