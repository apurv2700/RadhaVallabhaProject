...
export default function PrasadManagementScreen() {
  ...
  useEffect(() => {
    const fetchFestivals = async () => {
      try {
        const response = await fetch(`http://192.168.1.43:3000/api/prasadam/festivals`);
...
const handleDelete = async (festivalId) => {
  try {
    ...
    const response = await fetch(`http://192.168.1.43:3000/api/prasadam/festivals/soft-delete/${festivalId}`, {
      method: 'PATCH',
      headers: {
